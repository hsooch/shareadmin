...
<div class="tab_wraping">
    ...
    <div id="tab1" class="tab-content current">
        <h5 class="cont_Title">수강결과 조회(OUT)
            <div class="pg_location"><a>Go home</a>
                <span>&gt;</span> 학점교류 관리<span>&gt;</span> 수강결과 조회(OUT)
            </div>
        </h5>
		<div id="innTabContent">
			<div class="_articleContent" ng-show="$root.exchange.resultOut.pageViewType == 'list'">
				<div class="_border _write mt10">
					<div class="_inner">
						<fieldset>
							<legend>수강결과 조회(OUT)</legend>
											...
											<table class="w80">
												...
												<tbody>
													<tr>
                                                        ...
                                                        <td>
                                                            <select id="sch_kind2" ng-model="$root.exchange.resultOut.searchUserUnivArea" ng-change="getChildCdList('searchUserUnivArea', 'searchUserUniv')" class="_selectBox _fL wx150">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchUserUnivAreaList | orderBy:codeIdx:false">{{code.codeName}}</option>
                                                            </select>
                                                        </td>
                                                        <td colspan="2">
                                                            <select ng-model="$root.exchange.resultOut.searchUserUniv" class="_selectBox _fL w99">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchUserUnivList | orderBy:codeIdx:false">{{code.codeName}}</option>
                                                            </select>
                                                        </td>
                                                        <td><label for="mem_level" class="pl20">학년</label></td>
                                                        <td>
                                                            <select id="mem_level" ng-model="$root.exchange.resultOut.searchStudentGradeCode" class="_selectBox _fL wx100">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchStudentGradeCodeList | orderBy:codeIdx:false">{{code.codeName}}</option>
                                                            </select>
                                                        </td>
                                                        <td><label for="sch_kind" class="pl20">교류대학</label></td>
                                                        <td>
                                                            <select id="sch_kind" ng-model="$root.exchange.resultOut.searchUnivArea" ng-change="getChildCdList('searchUnivArea', 'searchUniv')" class="_selectBox _fL wx150">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchUnivAreaList | orderBy:codeIdx:false">{{code.codeName}}</option>
                                                            </select>
                                                        </td>
                                                        <td colspan="2">
                                                            <select ng-model="$root.exchange.resultOut.searchUniv" class="_selectBox _fL wx150">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchUnivList | orderBy:codeIdx:false">{{code.codeName}}</option>
                                                            </select>
                                                        </td>
													</tr>
													<tr>
                                                        ...
                                                        <td>
                                                            <select id="sch_year" ng-model="$root.exchange.resultOut.searchYear" class="_selectBox _fL wx150">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchYearList | orderBy:codeIdx:false">{{code.codeName}}년도</option>
                                                            </select>
                                                        </td>
                                                        <td><label for="sch_quarter" class="pl10">학기</label></td>
                                                        <td>
                                                            <select id="sch_quarter" ng-model="$root.exchange.resultOut.searchSemesterCode" class="_selectBox _fR wx100">
                                                                <option value="{{code.code}}" ng-repeat="code in $root.exchange.resultOut.searchSemesterCodeList | orderBy:codeIdx:false">{{code.codeName}}</option>
                                                            </select>
                                                        </td>
                                                        <td></td>
                                                        <td></td>
                                                        ...
                                                        <td colspan="3">
                                                            <select ng-model="$root.exchange.resultOut.searchType" class="_selectBox _fL wx100">
                                                            ...
                                                            <input kr-input type="text" ng-model="$root.exchange.resultOut.searchKey" placeholder="" class="w64">
                                                        </td>
                                                    </tr>
												</tbody>
											</table>
				...
				<div class="_listHead">
					<div class="_count">
						총 <strong>{{$root.exchange.resultOut.totalCnt}}</strong> 건이 검색되었습니다.
					</div>
					<div class="_search pr0">
						...
						<fieldset>
							...
							<select name="findType" ng-model="$root.exchange.resultOut.maxRowCnt" ng-change="changeMaxRowCnt()" class="_selectBox wx70">
				...
				<table class="_table _list w100">
					...
					<tbody ng-show="$root.exchange.resultOut.exchangeResultList.length > 0">
						<tr class="firTd" ng-repeat="exchangeResultInfo in $root.exchange.resultOut.exchangeResultList | orderBy:descRn:false">
				...
				<div ng-show="$root.exchange.resultOut.exchangeResultList.length > 0" id="exchangeResultListPaging" class="_paging"></div>
