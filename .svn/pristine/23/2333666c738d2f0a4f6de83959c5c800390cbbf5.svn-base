(function () {
    ...
    if(K_OSName == "Windows"){
        ...
        file = "http://" + url + "/kcase/client/" + "KSignCASE For HTML5_Windows_v1.3.13.exe";
    }
    else if(K_OSName == "Mac") {
        kcaseLibUrl = "//" + url + "/kcase/lib/mac_n_linux_lib";
        file = "http://" + url + "/kcase/client/" + "HTML5_KCaseAgentForMac.pkg";
    }
    else if(K_OSName == "Linux") {
        kcaseLibUrl = "//" + url + "/kcase/lib/mac_n_linux_lib";
        file = "http://" + url + "/kcase/client/" + "KCaseAgentForLinux-1.3.8-1.el6.i386.rpm";
    }
    ...
    var failAgentService = function() {
        alert("보안 모듈이 설치되지 않았거나 실행중이 아닙니다. 다운로드된 설치파일을 실행 후 다시 시도하시기 바랍니다.");
        window.location.href = file;
    };
    ...
    var cancelDialog = function() {
        //alert("취소되었습니다.");
    	$("div[id^='ngdialog']").each(function(){ $(this).remove(); });
    };
